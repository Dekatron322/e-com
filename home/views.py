...
def index(request):
    ...
    products_slider = Product.objects.all().order_by('?')[:3]
    products_latest = Product.objects.all().order_by('-id')[:4]
    products_daily = Product.objects.all().order_by('id')[:4]
...
def contactus(request):
    ...
    for rs in shopcart:
        added +=  rs.quantity
    context = {'setting':setting, 'total':total, 'added':added, 'form':form, 'category':category}
